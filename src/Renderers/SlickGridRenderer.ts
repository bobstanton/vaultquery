...
export class SlickGridRenderer extends BaseRenderer {
  ...
  static render(context: RenderContext): void {
    ...
    const initGrid = () => {
      ...
      try {
        ...
        const containerWidth = domContainer.offsetWidth || 800;
        const minTotalWidth = columns.reduce((sum, col) => sum + (col.minWidth || col.width || 120), 0);
        const shouldAllowScroll = minTotalWidth > containerWidth;
  ...
  private static createCurrentFormatter() {
    return (row: number, cell: number, value: unknown, columnDef: Column, dataContext: Record<string, unknown>) => {
