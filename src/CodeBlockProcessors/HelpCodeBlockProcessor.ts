...
export class HelpCodeBlockProcessor {
  ...
  public process(_source: string, el: HTMLElement, _ctx: MarkdownPostProcessorContext): void {
    el.empty();
    const container = el.createDiv({ cls: 'vaultquery-help' });

    vaultqueryHelp.render(container, this.createRenderContext());
  }
  ...
  private renderSchema(container: HTMLElement): void {
    ...
    if (api) {
      ...
      container.createDiv({
        text: 'Database not yet initialized. Schema will appear here once the plugin is ready.',
